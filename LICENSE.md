# Licence

Copyright © 2026 Benny Delaney-Brownlow. All rights reserved.

This licence covers both Bakehouse plugins for VCV Rack — the free
**Bakehouse** plugin (Whisk) and the commercial **Bakehouse Plus** plugin
(Whisk Expander) — together with their binaries, panel art, other bundled
resources, and this manual.

## Software

**Bakehouse** is distributed free of charge. You may download, install and use
it, in personal and commercial music, with no fee and no further permission.

**Bakehouse Plus** is a commercial plugin. Buying it grants you the same right
to install and use it, in personal and commercial music, on the computers you
use.

Neither plugin may be redistributed, resold, rented, sub-licensed, modified,
decompiled, disassembled or reverse-engineered, and no derivative work may be
made from it, without prior written permission from the copyright holder. The
source code is not published.

Music you make with these plugins is entirely yours. Nothing here claims any
interest in it.

## Names and artwork

"Bakehouse", "Whisk" and "Whisk Expander", and the panel artwork and component
graphics shipped with these plugins, are the property of the copyright holder
and may not be used to identify any other product.

## No warranty

These plugins are provided "as is", without warranty of any kind, express or
implied, including but not limited to the warranties of merchantability,
fitness for a particular purpose and non-infringement. In no event shall the
copyright holder be liable for any claim, damages or other liability arising
from, out of, or in connection with the software or its use.

## Contact

bennydelaneybrownlow@gmail.com
